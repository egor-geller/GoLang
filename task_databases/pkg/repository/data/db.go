...
func (r *ContactsRepositoryInDB) Save(contact model.Contact) (model.Contact, error) {
	err := r.db.QueryRow("INSERT INTO users(firstname,lastname,phone,email) VALUES($1,$2,$3,$4) returning id", contact.FirstName, contact.LastName, contact.Phone, contact.Email).Scan(&contact.ID)
...
func (r *ContactsRepositoryInDB) ListAll() (contact []model.Contact, err error) {
	rows, err := r.db.Query("select id, firstname,lastname,phone,email from users")
	if err != nil {
		return nil, err
	}
	for rows.Next() {
...
func (r *ContactsRepositoryInDB) GetByID(id uint) (p model.Contact, err error) {
	row := r.db.QueryRow("select id, firstname,lastname,phone,email from users where id = $1", id)
...
func (r *ContactsRepositoryInDB) GetByPhone(phone string) (p model.Contact, err error) {
	row := r.db.QueryRow("select id, firstname,lastname,phone,email from users where phone = $1", phone)
	err = row.Scan(&p.ID, &p.FirstName, &p.LastName, &p.Phone, &p.Email)
	if err != nil {
		return model.Contact{}, err
	}
...
func (r *ContactsRepositoryInDB) GetByEmail(email string) (p model.Contact, err error) {
	row := r.db.QueryRow("select id, firstname,lastname,phone,email from users where email = $1", email)
	err = row.Scan(&p.ID, &p.FirstName, &p.LastName, &p.Phone, &p.Email)
	if err != nil {
		return model.Contact{}, err
	}
...
func (r *ContactsRepositoryInDB) SearchByName(n string) (contact []model.Contact, err error) {
	row, err := r.db.Query("select id, firstname,lastname,phone,email from users where firstname like $1 or lastname like $1", n)
	if err != nil {
		return nil, err
	}
	for row.Next() {
